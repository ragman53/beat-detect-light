...
void ofxbeatdetect::audioReceived(float* input, int bufferSize){
...
// ビートそのものがあったか
bool ofxbeatdetect::isBeat(int subband)
{
    return fftSubbands[subband] > averageEnergy[subband]*beatValueArray[subband];
}

bool ofxbeatdetect::isKick()
{
    return isBeat(0);
}

bool ofxbeatdetect::isSnare()
{
    int low = 1;
...
bool ofxbeatdetect::isBeatRange(int low, int high, int threshold)
{
    int num = 0;
